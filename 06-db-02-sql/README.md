...
12.50. - Приблизительная стоимость запуска. Это время, которое проходит, прежде чем начнётся этап вывода данных.
.12.50 - Приблизительная общая стоимость. Она вычисляется в предположении, что узел плана выполняется до конца, то есть возвращает все доступные строки (кроме случаев с LIMIT).
rows=249 - Ожидаемое число строк, которое должен вывести этот узел плана.
...
```text
docker exec -t postgresql_1_pgdb_1_1 pg_dump -U postgres t    est_db -f /var/lib/postgresql/backups/dump_test.sql
```
...
docker exec -i postgresql_1_pgdb_2_1 psql -U postgres -d test_db -f /var/lib/postgresql/backups/dump_test.sql
```
